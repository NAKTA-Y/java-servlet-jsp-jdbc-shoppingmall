    ...
    <main class="w-full min-h-screen py-1 md:w-2/3 lg:w-3/4">
        <div class="bg-white shadow">
            <div class="px-4 py-6 sm:px-8 sm:py-10">
                <div class="flow-root">
                    <ul class="-my-8">
                        <c:forEach var="pointDetail" items="${requestScope.pointDetails}">
                            <c:choose>
                                <c:when test="${pointDetail.type eq 'POINT_USE'}">
                                    <li class="flex flex-col space-y-3 py-6 text-left sm:flex-row sm:space-x-5 sm:space-y-0">
                                        <div class="relative flex flex-1 flex-col justify-between">
                                            <div class="sm:col-gap-5 sm:grid sm:grid-cols-2">
                                                <div class="pr-8 sm:pr-5">
                                                    <p class="text-base font-semibold text-gray-900">${pointDetail.createdAt}</p>
                                                    <p class="mx-0 mt-1 mb-0 text-sm text-red-400">사용</p>
                                                </div>
                                            </div>

                                            <div class="absolute top-0 right-0 flex sm:bottom-0 sm:top-auto">
                                                <p class="inline-block text-red-600 font-semibold py-2 px-4 rounded transition duration-300 ease-in-out">
                                                    <fmt:formatNumber value="${pointDetail.volume}"/>
                                                </p>
                                            </div>
                                        </div>
                                    </li>
                                    ...
                                    <li class="flex flex-col space-y-3 py-6 text-left sm:flex-row sm:space-x-5 sm:space-y-0">
                                        <div class="relative flex flex-1 flex-col justify-between">
                                            <div class="sm:col-gap-5 sm:grid sm:grid-cols-2">
                                                <div class="pr-8 sm:pr-5">
                                                    <p class="text-base font-semibold text-gray-900">${pointDetail.createdAt}</p>
                                                    <p class="mx-0 mt-1 mb-0 text-sm text-blue-400">적립</p>
                                                </div>
                                            </div>

                                            <div class="absolute top-0 right-0 flex sm:bottom-0 sm:top-auto">
                                                <p class="inline-block text-teal-600 font-semibold py-2 px-4 rounded transition duration-300 ease-in-out">
                                                    +<fmt:formatNumber value="${pointDetail.volume}"/>
                                                </p>
                                            </div>
                                        </div>
